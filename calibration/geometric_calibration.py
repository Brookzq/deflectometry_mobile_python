...
def detectChecker(img, patternSize, debug=True):
    ...
    if ret:
        ...
        if debug:
            ...
            width = 1100
            height = int(img.shape[0] * 1100 / img.shape[1])
...
def reProjAruco(img, camMat, distCoeffs, rvec, tvec, cornersAruco, markerLength, arucoBoardHeight= 0.083, arucoBoardWidth= 0.125):
...
def calib(imgPath, camMtx, dist, half_length, half_height, displayScaleFactor, checker_col_row, marker_size):
    ...
    for i in range(1):
        ...
        width = 1100
        height = int(img_axis.shape[0] * 1100 / img_axis.shape[1])
        ...
        width = 1100
        height = int(img_rep.shape[0] * 1100 / img_rep.shape[1])
...
def geoCalib(imgFile, imgUndistortFile, half_length, half_height, marker_size, cameraMatrix, disCoeffs, ret, displayScaleFactor, checker_col_row):
    ...
    if len(markerCorners) > 0:
        ...
        cv2.aruco.drawDetectedMarkers(img, markerCorners, markerIDs, [0,170,0])
    ...
    half_length_disp = 1100 * displayScaleFactor
